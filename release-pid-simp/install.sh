...
case "$ACTION" in
    install)
    	# Config
    	RULE_FILE="/etc/udev/rules.d/fanctrl.rules"

	echo "Creating udev rule for fanctrl..."

	sudo bash -c "cat > $RULE_FILE" <<EOF
	KERNEL=="ttyUSB*", ATTRS{devpath}=="1.3.1", ATTRS{idVendor}=="1a86", ATTRS{idProduct}=="5523", SYMLINK+="fanctrl", MODE:="777", GROUP="dialout"
EOF
	...
	echo "Done. Check with: ls -l /dev | grep ttyUSB"
